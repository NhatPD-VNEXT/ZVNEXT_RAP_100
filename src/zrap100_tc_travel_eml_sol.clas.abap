"! @testing BDEF:ZRAP100_R_TravelTP_sol
CLASS zrap100_tc_travel_eml_sol DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS ZRAP100_TC_TRAVEL_EML_SOL IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'ZRAP100_R_TravelTP_sol' ) ) ).
  ...
  METHOD create_with_action.
    ...
    SELECT * FROM ZRAP100_R_TravelTP_sol INTO TABLE @DATA(lt_travel). "#EC CI_NOORDER
    "#EC CI_ALL_FIELDS_NEEDED
    "#EC CI_NOWHERE
    ...
    cl_abap_unit_assert=>assert_not_initial( msg = 'travel-id' act = lt_travel[ 1 ]-TravelID ). "#EC CI_NOORDER
    " assert that the action has changed the overall status
    cl_abap_unit_assert=>assert_equals( msg = 'overall status' exp = 'A' act = lt_travel[ 1 ]-OverallStatus ). "#EC CI_NOORDER
    " assert the discounted booking_fee
    cl_abap_unit_assert=>assert_equals( msg = 'discounted booking_fee' exp = '16' act = lt_travel[ 1 ]-BookingFee ). "#EC CI_NOORDER
